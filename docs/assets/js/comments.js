...
// This file needs to be loaded as a module to use Firebase
...
(async function() {
  ...
  function getPageId() {
    return window.location.pathname.replace(/\//g, '_').replace(/\.html$/, '').replace(/\.md$/, '') || 'home';
  }
  ...
  async function loadComments() {
    ...
    try {
      ...
      snapshot.forEach(docSnap => {
        html += createCommentHTML(docSnap.data(), docSnap.id);
      });

...
private struct CalculatorContent: View {
    ...
    private func resultCard(_ value: Double) -> some View {
        let interp = tool.interpretation?.first { Int(value) >= $0.min && Int(value) <= $0.max }
        let color: Color = {
            switch interp?.color {
            case "green": .green
            case "yellow": .yellow
        ...
        return GlassCard(cornerRadius: AppTheme.smallCornerRadius) {
            VStack(spacing: 12) {
                ...
                // Copy button
                Button {
                    var text = "\(tool.title): \(formatResult(value))"
                    if let unit = tool.resultUnit { text += " \(unit)" }
                    if let interp { text += " — \(interp.label)" }
    ...
    private func calculateResult() {
        guard let formula = tool.formula else { return }
        ...
        let computed: Double? = {
            ...
            case "anion-gap":
                ...
                let hco3 = vals["bicarb"] ?? 0
                return na - (cl + hco3)

            case "bmi-calculator":
                let weight = vals["weight"] ?? 0
                let heightCm = vals["height"] ?? 170
            ...
            case "ivig-dose":
                ...
                if regimen == "2-day rapid" {
                    return weight * 1.0 // 1 g/kg/day × 2 days, show daily
                }
                return weight * 0.4 // 0.4 g/kg/day × 5 days, show daily
            ...
            case "steroid-equivalence":
                ...
                let predEquiv: Double
                switch drug {
                case "Methylprednisolone": predEquiv = dose * (5.0 / 4.0)
                case "Dexamethasone": predEquiv = dose * (5.0 / 0.75)
                case "Hydrocortisone": predEquiv = dose * (5.0 / 20.0)
                default: predEquiv = dose // Prednisone
                }
                return predEquiv
